  ...
  return <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold text-foreground">CHECKLIST</span>
        </div>
      ...
      <div className="space-y-2">
        {tasks.map((task, index) => <div key={task.id} className={cn("flex items-center gap-3 p-3 rounded-lg bg-card border border-border transition-all duration-300 animate-fade-slide-in", task.completed && "task-completed bg-muted")} style={{
        animationDelay: `${index * 50}ms`
      }}>
            <Checkbox id={task.id} checked={task.completed} onCheckedChange={() => handleToggle(task.id)} className="border-primary data-[state=checked]:bg-status-green data-[state=checked]:border-status-green" />
            <label htmlFor={task.id} className={cn("text-sm font-medium cursor-pointer flex-1", task.completed && "line-through opacity-60")}>
              {task.label}
            </label>
          </div>)}
      </div>
