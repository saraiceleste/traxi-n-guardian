...
function getStatusFromScore(score: number) {
  if (score < 0.5) return { emoji: "🚨", colorClass: "text-status-red" };
  if (score < 0.75) return { emoji: "👁️", colorClass: "text-status-yellow" };
  return { emoji: "🟢", colorClass: "text-status-green" };
}
      ...
      <div className="grid grid-cols-2 gap-2">
        {risks.slice(0, 4).map((risk) => {
          ...
          return (
            <div
              key={risk.rank}
              className="flex items-center gap-1 text-xs p-2 rounded-lg bg-muted/50"
            >
